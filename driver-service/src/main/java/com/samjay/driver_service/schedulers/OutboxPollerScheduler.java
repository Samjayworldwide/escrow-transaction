package com.samjay.driver_service.schedulers;

import com.samjay.driver_service.entities.OutboxEvent;
import com.samjay.driver_service.enumerations.OutboxEventStatus;
import com.samjay.driver_service.repositories.OutboxEventRepository;
...
public class OutboxPollerScheduler {
    ...
    private void handleFailure(OutboxEvent event, String errorMessage) {
        ...
        if (event.getStatus() == OutboxEventStatus.DEAD_LETTER) {
            ...
            log.error(
                    "Outbox event moved to DEAD_LETTER [id={}, type={}, aggregateId={}]. " + "Manual intervention required.",
                    event.getId(),
                    event.getEventType(),
